...
GAMES_WORD =   ['LORVAANT', 'EEFIFRRE',  'STATENEW',  'FAFI', 'RIOAM',  'ITFOTERT', 'CRAMINFTE']
GAMES_ANSWER = ['VALORANT', 'FREEFIRE',  'NEWSTATE',  'FIFA', 'MARIO', 'FORTNITE', 'MINECRAFT' ]
 
ran_num = randrange(0, (len(GAMES_WORD)))
jumbled_rand_word = GAMES_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(GAMES_WORD)))
        word.configure(text=GAMES_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == GAMES_ANSWER[ran_num]:
            points += 1
    ...
    def show_answer():
        ...
        if points >=1:
            ...
            ans_lab.configure(text=GAMES_ANSWER[ran_num])
        else:
