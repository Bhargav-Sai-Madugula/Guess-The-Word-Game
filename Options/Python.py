...
PYTHON_WORD = ['INTRP', 'ORMPIT', 'URNTER', 'SALSC', 'BDALAM', 'NUECONTI', 'GLALBO','ERTASS','RUET' ]

PYTHON_ANSWER = ['PRINT', 'IMPORT', 'RETURN', 'CLASS', 'LAMBDA', 'CONTINUE', 'GLOBAL','ASSERT','TRUE' ]
 
ran_num = randrange(0, (len(PYTHON_WORD)))
jumbled_rand_word = PYTHON_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(PYTHON_WORD)))
        word.configure(text=PYTHON_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == PYTHON_ANSWER[ran_num]:
            points += 1
    ...
    def show_answer():
        ...
        if points >=1:
            ...
            ans_lab.configure(text=PYTHON_ANSWER[ran_num])
        else:
